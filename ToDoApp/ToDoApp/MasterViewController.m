...
- (void)awakeFromNib {
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        ...
        //Set the preferred size for the view controller’s view.
...
//    if (!self.objects) {
  //      self.objects = [[NSMutableArray alloc] init];
    ...
    UIAlertAction *okAction = [UIAlertAction
                               ...
                               {
                                   //UITextField *nameItem = alertController.textFields[0];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"showDetail"]) {
        ...
        [controller setDetailItem:[ToDoItems objectAtIndex:indexPath.row]];
...
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        NSString *imageName = [ToDoItems objectAtIndex:indexPath.row];
